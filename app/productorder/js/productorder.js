		...
		if(["recieved","approved","packing","delivery","past"].indexOf(criteriastatus)<0){
			...
			if(criteriastatus=="recieved"){
			 ...
			 /////////////////////////////////////////////////
				...
				query.push({$unwind:"$suborder"})
				query.push({$match:{"suborder.productprovider.branchid":branchid,"suborder.status":{$in:statusarray[criteriastatus]}}})
				query.push({$project:{buyerpayment:"$suborder.buyerpayment",pickup_address:"$suborder.pickup_address",sellerpayment:"$suborder.sellerpayment",orderinstructions:"$suborder.orderinstructions", payment:1,preferred_delivery_date:1,createdate:1,suborderid:"$suborder.suborderid",products:"$suborder.products",suborder_price:"$suborder.suborder_price",billing_address:"$suborder.billing_address",delivery_address:"$suborder.delivery_address",deliverytype:"$suborder.deliverytype",deliverydatetime:{day:{$dayOfMonth:'$suborder.deliverydate'},month:{$month:'$suborder.deliverydate'},year:{$year:'$suborder.deliverydate'}},status:"$suborder.status",_id:0,consumer:1}})
				query.push({$sort:{createdate:1}})
			}
		 _getMySubOrders(self,userid,providerid,branchid,query)
		  ////////////////////////////////////////////////////
