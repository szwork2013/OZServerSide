...
var _validateConversationData = function(self,conversation_data,orderid,session_userid){
	if(conversation_data == undefined){
		self.emit("failedServiceConAddMsg",{"error":{"code":"AV001","message":"Please provide massagedata"}});
	}else if(conversation_data.massage == undefined){
		self.emit("failedServiceConAddMsg",{"error":{"code":"AV001","message":"Please provide massage"}});
	}else if(conversation_data.from == undefined){
		self.emit("failedServiceConAddMsg",{"error":{"code":"AV001","message":"Please provide sender"}});
	}else if(conversation_data.to == undefined){
		self.emit("failedServiceConAddMsg",{"error":{"code":"AV001","message":"Please provide receiver"}});
	}else{
...
var _checkOrderIdIsValidForConversation = function(self,orderid,session_userid){
	OrderModel.findOne({orderid:orderid}).lean().exec(function(err,order){
		if(err){
			...
			self.emit("failedServiceConAddMsg",{"error":{"code":"AU001","message":"Already Exist"}});
			// _addConversation(self,conversation_data,orderid);
		}else{
			self.emit("failedServiceConAddMsg",{"error":{"code":"AU001","message":"Wrong orderid"}});			
		}
	})
