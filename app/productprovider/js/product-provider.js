...
var getPayableRefundableExcelSheetForProvider=function(providerid,transactiondate,callback){
	...
	ProductOrderModel.aggregate({$project:{orderdate:{$add:["$order_placeddate",60*60*1000*5.5]},orderid:1,suborder:1,payment:1,status:1}},{$project:{orderid:1,suborder:1,payment:1,status:1,orderdate:{day:{$dayOfMonth:'$orderdate'},month:{$month:'$orderdate'},year:{$year:'$orderdate'}}}},{$unwind:"$suborder"},{$match:{"orderdate.day":day,"orderdate.month":month,"orderdate.year":year,status:{$ne:"waitforapproval"},"payment.mode":new RegExp("paytm","i"),"payment.STATUS": new RegExp("txn_success","i"),"suborder.productprovider.providerid":providerid}},{$project:{orderid:1,orderdate:1,suborderid:"$suborder.suborderid",status:"$suborder.status",suborder_price:"$suborder.suborder_price",providername:"$suborder.productprovider.providername",providerid:"$suborder.productprovider.providerid"}},function(err,createdsuborders){
		if(err){
			callback({error:{code:"ED001",message:"Database Issue"}});
			console.log("Database Error"+err);
		}else{
			//to get refundable transaction details order cancelled by vendor,ordercancelledy user,order rejected by vendor
			ProductOrderModel.aggregate({$project:{orderid:1,suborder:1,payment:1,status:1,order_placeddate:1}},{$unwind:"$suborder"},{$match:{"suborder.cancelrejectdate":{$ne:null}}}, {$project:{cancelrejectdate:{$add:["$suborder.cancelrejectdate",60*60*1000*5.5]},orderid:1,payment:1,status:1,suborder:1,order_placeddate:1}},{$project:{orderid:1,payment:1,status:1,suborder:1,order_placeddate:1,cancelrejectdate:{day:{$dayOfMonth:'$cancelrejectdate'},month:{$month:'$cancelrejectdate'},year:{$year:'$cancelrejectdate'}}}},{$match:{status:{$ne:"waitforapproval"},"payment.mode":/paytm/i,"payment.STATUS": new RegExp("txn_success","i"),"suborder.productprovider.providerid":providerid,"suborder.status":{$in:["cancelled","rejected","cancelledbyconsumer"]},"cancelrejectdate.day":day,"cancelrejectdate.month":month,"cancelrejectdate.year":year}},{$project:{orderid:1,orderdate:"$order_placeddate",cancelrejectdate:1,suborderid:"$suborder.suborderid",status:"$suborder.status",suborder_price:"$suborder.suborder_price",providername:"$suborder.productprovider.providername",providerid:"$suborder.productprovider.providerid"}},function(err,cancelledrjectedsuborder){
				if(err){
					callback({error:{code:"ED001",message:"Database Issue"}});
								console.log("Database Error::"+err);
				}else{
